...
const contactLinks = [
  ...
  },
  {
    ...
    href: LINKEDIN_URL,
    display: 'linkedin.com/in/laylarodas',
    external: true,
    icon: (
  ...
  {
    id: 'upwork',
    labelKey: 'contact.links.upwork',
    href: UPWORK_URL,
    display: 'Upwork',
    external: true,
    icon: (
      <span className="w-5 h-5 flex items-center justify-center text-xs font-bold">Up</span>
    ),
  },
]
...
function Contact() {
  ...
  const submitViaMailto = () => {
    ...
    window.location.href = `mailto:rodas.layla@gmail.com?subject=${subject}&body=${body}`
    setStatus('mailto')
          ...
          <div className="scroll-animate" style={{ transitionDelay: '200ms' }}>
            ...
            <div className="flex flex-col gap-4">
              {contactLinks.slice(1).map((link) => (
                ...
                >
                  {link.icon}
                  <span className="font-mono text-sm">{t(link.labelKey)}</span>
                </a>
              ))}
            </div>
          </div>
